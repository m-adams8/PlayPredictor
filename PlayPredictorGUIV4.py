...
class PlayPredictorGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.CLOSEW = 7      # Default value for close game when winning
        self.CLOSEL = -7     # Default value for close game when losing
    ...
    def initUI(self):
        ...
        self.score_differential = QLineEdit()
        ...
        get_play_form.addRow('Score Differential:', self.score_differential)
        ...
        # Connect coach selection to update score differential
        self.coach_select_combo.currentTextChanged.connect(self.update_score_differential)
        ...
        self.play = QLineEdit()
        self.score_differential_add = QLineEdit()
        self.score_differential_add.setPlaceholderText("Score differential (positive if winning, negative if losing)")
        ...
        add_play_form.addRow('Score Differential:', self.score_differential_add)
        ...
        # Close game settings
        self.closew_spin = QSpinBox()
        self.closew_spin.setMinimum(-99)
        self.closew_spin.setMaximum(99)
        self.closew_spin.setValue(self.CLOSEW)
        self.closew_spin.setStyleSheet(spinbox_style)
        settings_form.addRow(QLabel('Close Game Winning Threshold:'), self.closew_spin)
        
        self.closel_spin = QSpinBox()
        self.closel_spin.setMinimum(-99)
        self.closel_spin.setMaximum(99)
        self.closel_spin.setValue(self.CLOSEL)
        self.closel_spin.setStyleSheet(spinbox_style)
        settings_form.addRow(QLabel('Close Game Losing Threshold:'), self.closel_spin)
    ...
    def update_date_and_play_number(self):
        """
        Update the game date, play number, and score differential fields based on the selected coach.
        ...
        if coach_name:
            last_date, last_play_number, last_score_differential = self.get_last_play_info(coach_name)
            ...
            # Set score differential to last used value if not already set
            if not self.score_differential_add.text().strip():
                if last_score_differential is not None:
                    self.score_differential_add.setText(str(last_score_differential))
                    #self.score_differential.setText(str(last_score_differential))
    
    def update_score_differential(self):
        """
        Update the score differential field in the Get Play tab based on the selected coach.
        """
        coach_name = self.coach_select_combo.currentText()
        if coach_name:
            last_date, last_play_number, last_score_differential = self.get_last_play_info(coach_name)
            
            # Set score differential to last used value if not already set
            if not self.score_differential.text().strip():
                if last_score_differential is not None:
                    self.score_differential.setText(str(last_score_differential))
    ...
    def get_last_play_info(self, coach_name):
        """
        Get the last used date, highest play number, and last score differential for a coach.
        
        Args:
            coach_name (str): Name of the coach
            
        Returns:
            tuple: (last_date, last_play_number, last_score_differential) or 
                   (None, None, None) if no plays exist
        """
        db_path = self.get_coach_db_path(coach_name)
        ...
        try:
            ...
            # Get the most recent date, its corresponding highest play number, and score differential
            cursor.execute("""
                SELECT Game_Date, MAX(Play_Number) as Last_Play_Number, Score_Differential
                FROM Coach_Scheme
                GROUP BY Game_Date
            ...
            last_date, last_play_number, last_score_differential = result
            
            conn.close()
            
            return (last_date, 
                    last_play_number if last_play_number else 0, 
                    last_score_differential if last_score_differential is not None else 0)
    ...
    def get_play(self):
        ...
        try:
            ...
            # Add score differential condition if provided
            if self.score_differential.text():
                if int(self.score_differential.text()) > CLOSEW:
                    query += " AND Score_Differential > " + str(CLOSEW)
                elif int(self.score_differential.text()) < CLOSEL:
                    query += " AND Score_Differential < " + str(CLOSEL)
                else:
                    query += " AND Score_Differential <= " + str(CLOSEW) + " AND Score_Differential >= " + str(CLOSEL)
    ...
    def add_play(self):
        ...
        try:
            ...
            required_fields = {
                ...
                'Play': self.play.text(),
                'Score_Differential': self.score_differential_add.text()
            }
            ...
            # Insert the play
            cursor.execute("""
                INSERT INTO Coach_Scheme (Game_Date, Play_Number, Quarter, Down, Distance, Yard_Line, Personel, Formation, Play, Score_Differential)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                self.game_date.text(),
                ...
                self.formation.text(),
                self.play.text(),
                self.score_differential_add.text()
            ))
            ...
            self.play.clear()
            self.score_differential_add.clear()
    ...
    def apply_settings(self):
        ...
        try:
            ...
            self.CLOSEW = self.closew_spin.value()
            self.CLOSEL = self.closel_spin.value()
            ...
            if self.BACKED_UP <= -1 or self.REDZONE <= -1:
